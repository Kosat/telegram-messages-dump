...
class DumpMetadata:
    ...
    def _loadFromFile(self):
        ...
        try:
            ...
            raise MetadataError(
                'Unable to open the metadata file "{}". {}'.format(
                    self.meta_file_path, ex.strerror
                )
            ) from ex
        except ValueError as ex:
            raise MetadataError(
                'Unable to load the metadata file "{}". AttributeError: {}'.format(
                    self.meta_file_path, ex
                )
            ) from ex
    ...
    def delete_meta_file(self):
        ...
        try:
            ...
            if ex.errno != errno.ENOENT:
                raise MetadataError(
                    "Failed to delete old metadata file. {}".format(ex.strerror)
                )
    ...
    def save_meta_file(self, new_dict):
        ...
        try:
            ...
            if DumpMetadata.CHAT_NAME in new_dict:
                self._meta_dict[DumpMetadata.CHAT_NAME] = new_dict[
                    DumpMetadata.CHAT_NAME
                ]
            if DumpMetadata.LAST_MESSAGE_ID in new_dict:
                self._meta_dict[DumpMetadata.LAST_MESSAGE_ID] = new_dict[
                    DumpMetadata.LAST_MESSAGE_ID
                ]
