...
class TelegramDumper(TelegramClient):
    ...
    def run(self):
        ...
        sprint(
            "{} messages were successfully written in the resulting file. Done!".format(
    ...
    def _getChannel(self):
        ...
        if name.startswith(JOIN_CHAT_PREFIX_URL):
            ...
            try:
                ...
                if peer:
                    sprint(
                        'Invitation link "{}" resolved into channel id={}'.format(
                            name, peer.id
                        )
                    )
                    return peer
        ...
        if name.startswith("@"):
            ...
            try:
                ...
                if peer.chats is not None and peer.chats:
                    sprint(
                        'Chat name "{}" resolved into channel id={}'.format(
                ...
                if peer.users is not None and peer.users:
                    sprint(
                        'User name "{}" resolved into channel id={}'.format(
                ...
                self.logger.debug(
                    'Failed to resolve "%s" as @-chat-name. %s',
                    self.settings.chat_name,
                    ex,
                    exc_info=self.logger.level > logging.INFO,
                )
        ...
        for dialog in dialogs:
            if dialog.name == name:
                sprint(
                    'Dialog title "{}" resolved into channel id={}'.format(
    ...
    def _fetch_messages_from_server(self, peer, buffer):
        ...
        for _ in range(0, 5):
            try:
                ...
                # before failing
                messages = self.get_messages(peer, limit=100, offset_id=self.id_offset)

                if messages.total > 0 and messages:
                    sprint(
                        "Processing messages with ids {}-{} ...".format(
        ...
        for msg in messages:
            self.exporter_context.is_first_record = (
                True if self.msg_count_to_process == 1 else False
            )
    ...
    def _write_final_file(self, buffer, temp_files_list_meta):
        ...
        with codecs.open(
            self.settings.out_file, result_file_mode, "utf-8"
        ) as resulting_file:
            if self.settings.is_addbom:
                resulting_file.write(codecs.BOM_UTF8.decode())

            self.exporter.begin_final_file(resulting_file, self.exporter_context)
            ...
            self._merge_temp_files_into_final(resulting_file, temp_files_list_meta)
    ...
    def _check_preconditions(self):
        ...
        if self.settings.is_incremental_mode:
            ...
            sprint(
                'Dumping messages newer than {} using "{}" dumper.'.format(
            ...
            sprint(
                'Dumping {} messages into "{}" file ...'.format(
                    "all"
                    if self.msg_count_to_process == sys.maxsize
                    else self.msg_count_to_process,
                    out_file_path,
                )
            )
